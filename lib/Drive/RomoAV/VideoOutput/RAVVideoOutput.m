...
- (id)init
{
    ...
    if (self) {
        ...
        _destinationFrame = av_frame_alloc();
        _outputBuffer = NULL;
        ...
        _codecCtx->width = 0;
        _codecCtx->height = 0;
        _codecCtx->flags2 |= AV_CODEC_FLAG2_FAST;
            ...
            if (avPacket.data) {
                ...
                if (!_isInitialized) {
                    if (_codecCtx->width > 0 && _codecCtx->height > 0) {
                        int outputBufferLength = avpicture_get_size(AV_PIX_FMT_NV12,
                                                                 _codecCtx->width,
                                                                 _codecCtx->height);

                        _outputBuffer = av_malloc(outputBufferLength);

                        avpicture_fill((AVPicture *) _destinationFrame, _outputBuffer, AV_PIX_FMT_NV12, _codecCtx->width, _codecCtx->height);
